...
def printImportant():
    ...
    ) = DNA_Generator.returnData()

    print(bcolors.OK + "--------YOU ARE RUNNING PREVIEW.py--------" + bcolors.RESET)
    print(
        "*Please Note: Running this test will have no effect on your config.py settings or the state of Blend_My_NFTs."
    )
    print("")
    print(bcolors.WARNING + "---config.py SETTINGS---" + bcolors.RESET)
    print(
        "NFTs Per Batch(nftsPerBatch): "
        + bcolors.WARNING
    ...
    print(
        "Image File Format(imageFileFormat): "
        + bcolors.WARNING
        + config.imageFileFormat
        + bcolors.RESET
    )
    print(
        "Operating system: " + bcolors.WARNING + str(platform.system()) + bcolors.RESET
    )
    print("Save Path(save_path): " + bcolors.WARNING + config.save_path + bcolors.RESET)
    print(
        "Possible DNA Combinations(possibleCombinations): "
        + bcolors.WARNING
    ...
    print(
        ...
        + bcolors.WARNING
        + str(Number_Of_Possible_Batches)
        + bcolors.RESET
    )

    if remainder > 0:
        print(
            "One incomplete batch will have "
            + bcolors.WARNING
            + str(remainder)
            + bcolors.RESET
            + " DNA in it."
    ...
    print("\nSettings:")
    print(
        "Reset viewport(enableResetViewport): "
        + bcolors.WARNING
        + str(config.enableResetViewport)
        + bcolors.RESET
    )
    print(
        "3D Models(enable3DModels): "
        + bcolors.WARNING
        + str(config.enable3DModels)
        + bcolors.RESET
    )
    print("")
    ...
    if config.enable3DModels:
        print(
            "3D Model File Format(objectFormatExport): "
            + bcolors.WARNING
    ...
    print(
        "Generate Colours(enableGeneration): "
        + bcolors.WARNING
        + str(config.enableGeneration)
        + bcolors.RESET
    )
    print("")
    print(
    ...
    if config.enable3DModels:
        print(
            bcolors.WARNING
            + "Cannot run Render Test when enable3DModels = True"
            + bcolors.RESET
        )
