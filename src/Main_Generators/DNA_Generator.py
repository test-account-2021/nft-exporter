...
def returnData():
    ...
    def attributeData(attributeVariants):
        ...
        for i in attributeVariants:
            ...
            def getOrder_rarity(i):
                ...
                del a[0]
                # if we have color (last element on the attribute name)
                # we use the color rarity
                if len(a) > 2:
                    a[1] = config.materialRarityList[int(a[2]) - 1]
                return list(a)

            name = getName(i)
            orderRarity = getOrder_rarity(i)
            print(
                "PEDRO Processing attribute variant: "
                + name
                + " with order rarity: "
                + str(orderRarity)
            )
    ...
    def numOfCombinations(hierarchy):
        ...
        try:
            ...
            print(
                bcolors.ERROR
                + "ERROR:\nnftsPerBatch in config.py needs to be a positive integer."
                + bcolors.RESET
            )
...
def send_To_Record_JSON():
    ...
    try:
        ...
        print(
            bcolors.OK
            + f"{len(DNAList)} NFT DNA sent to NFTRecord.json in %.4f seconds.\n"
